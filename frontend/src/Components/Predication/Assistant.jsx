...
import './GetStart.css'; // Assuming you have a CSS file for styling
...
export default function AI() {
  ...
  const addUser = () => {
    if (newUser.trim()) {
      setUsers([...users, { id: Date.now(), name: newUser, fines: [] }]);
      setNewUser("");
    }
  };
  ...
  const confirmDeleteFine = () => {
    ...
    console.log("Selected Fine Index:", selectedFineIndex);

    const updatedUsers = users.map((user) => {
      if (user.id === selectedUserId) {
        user.fines.splice(selectedFineIndex, 1);
      }
      return user;
    });
    setUsers(updatedUsers);
    ...
    <div>
      ...
      <Box sx={{ padding: 3, fontFamily: "Arial, sans-serif" }}>
        ...
        {filteredUsers
          ...
          .sort((a, b) => b.id - a.id)
          .map((user) => (
            <Card key={user.id} sx={{ mb: 4, boxShadow: 3 }}>
              <CardContent>
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={8}>
                    <Typography variant="h6">{user.nic}</Typography>
                    <Typography variant="h6">{user.name}</Typography>
                    <Typography variant="h6">{user.phone}</Typography>
                    <Typography variant="body2" color="textSecondary" sx={{ mt: 2 }}>
                      Predicted Next Fine: {AI_PredictFine[Math.floor(Math.random() * AI_PredictFine.length)].type}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                            ...
                            <Page>
                              ...
                              {user.fines && user.fines.length > 0 ? (
                                user.fines.map((fine, index) => (
                                  <Text key={index}>
                                    {fine.type} - ${fine.amount}
                                  </Text>
                                ))
                              ) : (
                                <Text>No fines available</Text>
                              )}
                            </Page>
                          </Document>
                        }
        ...
        <Dialog open={fineDialogOpen} onClose={() => setFineDialogOpen(false)}>
          <DialogTitle>Confirm Delete</DialogTitle>
          <DialogContent>
            <DialogContentText>Are you sure you want to delete this fine?</DialogContentText>
          </DialogContent>
          <DialogActions>
